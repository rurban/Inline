...
Instead of specifying the source code as a here-document string, you may want to put it at the end of your script, after the C<__END__> statement. Then you can tell Inline to read it in from the C<DATA> filehandle, like this:
...
This syntax makes use of the source-file keyword, C<'DATA'>, and a special marker: C<__C__>. 'C' is the programming language you are binding to. Inline will read through the DATA filehandle and look for the special marker. It will use everything up until the next marker or the end of the file. 

NOTE: The 'DATA' keyword can be ommitted if there are no extra parameters following it.

If you are writing a module, you can also use the DATA section for POD and AutoLoader subroutines. Just be sure to put them before the first Inline marker. If you install the helper module L<Inline::Filters>, you can even use POD inside your Inline code. You just have to specify a filter to strip it out.
...
As a convenience, Inline copies your latest error directory to C<DIRECTORY/errors> and strips all of the MD5 hash codes out of the names. More debugging improvements are coming in future releases.
...
IMPORTANT NOTE: It has been determined that creating modules for CPAN using the above technique is doable, but not optimal. The basic problem is that there is no way to automatically remove old executable objects on subsequent releases. There are also some additional subtle problems. So, my official stance is to not use Inline for CPAN modules until Inline release 0.40 or greater. These issues will be resolved at that release level. This technique is still perfectly fine for personal use, if your not too concerned about have a few harmless artifacts in your site lib tree.
...
If the C<SITE_INSTALL> option is in effect, then Inline will only use C<./_Inline/> to build in, and the C<$Config{installsitearch}> directory to install the executable in. This option is intended to be used in modules that are to be distributed on the CPAN, so that they get installed in the proper place.
...
While Inline does attempt to clean up after itself, there is currently no functionality to remove a shared object when a new version is compiled. This will be addressed in version 0.40 and higher.
