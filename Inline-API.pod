...
This subroutine is responsible for doing the parsing and compilation of the Foo++ source code. The Inline is passed as the only argument. All pertinent information will be stored in this object. C<build()> is required to create a shared object of a specific name, or to craok with an appropriate error message. 
