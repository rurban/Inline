...
Inline has always been intended to work with lots of different programming languages. Many of the details can be shared between implementations, so that C<Inline::Java> has a similar interface to C<Inline::Fortran>. All of the common code is in C<Inline.pm>. 
...
To prevent naming clashes with future version of Inline you use a subhash under the key 'Foo' (The name of your language). This is the convention.
...
Flag indicating whether or not Inline should call the C<Inline::Foo::build()> method even if the code has already been compiled. (0 or 1)
